...
необходимый для локализации, сразу во все ```end-поинты```
нашего приложения:
...
хотим читать сообщения от сервера.

![accept_lang](img/accept_lang.jpg)

А здесь показан пример того, как благодаря внедрению в роутер
функции ```get_user_settings```, во все ```end-поинты``` роутера передаются
...
async def get_item_by_id(row_id: int, current_user_settings = Depends(get_user_settings)) -> AuthorGetItemSchema:
    ...
    return item
```
При этом авторизация осуществляется непосредственно
внутри функции ```get_user_settings```. Благодаря такому 
подходу, при запуске ```end-поинтов``` данного роутера
пользователь авторизуется только один раз.
...
просто за счет добавления ```JSON``` файла с
соответствующими локализационными данными.
...
функцией ```[rule_for]```. Максимальное количество правил
соответсвует количеству полей ```pydantic-схемы```.
Таким образом для каждого поля может быть создано
только одно правило (рекомендуется).

Для каких именно полей создавать правила, и в каком
порядке их располагать внутри карты
валидации не имеет значения.
...
таким образом нашу карту валидации !!!
```
...
Под капотом класса ```BaseValidator``` существует переменная
```property_value```, которая получает своё значение при выполнении
лямбда-функции из ```rule_for```.
...
ей необходимо передать ссылку на функцию, в которой
реализована логика проверки. Пример такой функции приведён ниже.

Функция ```is_unique_name_author``` делает запрос к базе данных,
...
async def is_unique_name_author(item, v: Any, session) -> bool:
    ...
    return True
```
При использовании готовых проверочных методов
класса ```BaseValidator``` таких как ```is_empty```, ```greater_than_or_equal```,
`precision_scale` и прочих, нет необходимости писать
функции для реализации логики проверки. Вся логика
там уже есть. Поэтому, вместо ```must``` лучше, по-возможностси, 
использовать их.
...
Разработаны роутеры get-запросы на получение сводных данных для отчётов

Сводные отчеты по распечатке справочников
...
при написании запроса необходимо писать filter, а внутри where

Как работать с SQLAlchemy
