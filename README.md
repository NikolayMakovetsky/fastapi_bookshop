...
Авторизация пользователей реализована при помощи FastAPIUsers.
- ORM: SQLAlchemy
- Transport: Cookie
- Strategy: JWT

Описание трех роутеров: регистер, логин, логаут

картинка роутеров авторизации

указать какие поля задавать при создании пользователя

При входе в систему вернется код 204

для выхода из системы нажмите логаут
...
в этой функции вычисляется язык сервера

отвечает за вычисление языка

настроены два языка.

если первые 2 символа совпадает со списком, будет выбран нужный язык

в противном случае будет выбран английский язык

get_user_settings - привязана к каждому роутеру, который требует авторизации
сама авторизация "подключается" внутри get_user_settings

get_user_settings также используется для передачи языка в роутер
...
Логализация сделана в виде словаря, который грузится в начале проекта

Если нужно добавить локализационное сообщение или

добавить язык, необходимо добавить еще один файл

```./api/core/resources/localize```
- messages.en.json
- messages.ru.json
...
```
что делает функция (документация по функции)

расшифровка передаваемых параметров
...
импорт через нижнее подчеркивание
```
какие параметры принимает, что возвращает
...
-------------------------------------------------------
    ...
    MUST позволяет задать асинхронную валидационную функцию (lambda)
...
self.rule_for().must().when().message()
self.rule_for().must().message().must().message()
self.rule_for().must().when().message().must().when().message()
```
